...
exports.createBlog = function (req, res) {
  ...
  const { blogCID, title, heroImg, postTime, nLikes, numComments, author } =
    data;

  Blog.findOne({ blogCID }).then((blog) => {
    if (blog) {
...
// Get All BLog by CID
exports.getAllBlogByWallet = function (req, res) {
  const { walletAddress } = req.body;

  Blog.find({ walletAddress })
    .then((blog) => {
      if (!blog) {
        error = "Blog Not Found";
        return res.status(404).json({ error });
